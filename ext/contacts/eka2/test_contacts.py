...
def script11():
    ...
    print "num of groups:" + str(len(db.groups))
    for grp_id in db.groups:
        print "group id:" + str(db.groups[grp_id].id)
        print "group name:" + str(db.groups[grp_id].name)
        print "****************"
        
    # delete all groups
    for grp_id in db.groups:
        del db.groups[grp_id]
        
    # add some groups    
        
    db.groups.add_group("test group 1") # create group and set name
    grp = db.groups.add_group() # create group without name
    grp.name = "test group 2" # set name
    grp = db.groups.add_group("test group 3")
    grp.name = "test group 4" # rename 
    
    print "this contact should be a group (should print 1)"
    print db[grp.id].is_group
    
    print "num of groups now:" + str(len(db.groups))
    for grp_id in db.groups:
        print "group id:" + str(db.groups[grp_id].id)
        print "group name:" + str(db.groups[grp_id].name)
        print "****************"
    ...
    # create test entry
    c = db.add_contact()
    c.add_field("first_name", "GROUP TEST ENTRY")
    c.commit()    
    
    print "this contact should not be a group (should print 0)"
    print c.is_group
    
    # is the group test entry in the group (it should not be)?
    if c.id in grp:
        print "GROUP TEST ENTRY FOUND, NOK!!!"    
    
    # add entires into group
    for entry_id in db:
        grp.append(entry_id)
        
    # is the group test entry in the group NOW?
    if c.id in grp:
        print "GROUP TEST ENTRY FOUND, OK"
        
    print "contacts in the group:"
    print list(grp)
    
    print "first contact id in the group:"
    print grp[0]
    
    # delete the first item in the group
    del grp[0]
    ...
    # group test entry should now have been removed from the group"
    if c.id in grp:
        print "GROUP TEST ENTRY FOUND, NOK!!!"
        
    print "contacts in the group (should be empty list):"
    print list(grp)

# script 12, open default database and display field type and field value
# for each contact.This test case is added to test the fix for bug no.1954740 
def script12():
    db = contacts.open()
    for x in db:
        print db[x]
        # Try to access the middle name field, using for example:
        for field in db[x]:
